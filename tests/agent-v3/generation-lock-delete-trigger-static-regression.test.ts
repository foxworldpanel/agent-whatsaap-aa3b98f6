...
const guard=readFileSync(resolve(process.cwd(),"supabase/migrations/20260914451500_generation_lock_delete_guard_welcome_funnel.sql"),"utf8");
describe("generation lock direct-delete guard",()=>{
 ...
 it("also fences running Welcome Funnel ownership",()=>{expect(guard).toContain("FROM public.welcome_funnel_execution_state f");expect(guard).toContain("f.status='running'");});
});
