...
describe("Customer Turn dispatcher maintenance budget",()=>{
 it("shares the same deadline across maintenance and claim phases",()=>{
  expect(source).toContain("const elapsedMs=()=>Date.now()-startedAt");
  expect(source).toContain("const withinBudget=()=>elapsedMs()<budgetMs");
  ...
  expect(source).toContain("if(!canStartRuntime())break");
 });
