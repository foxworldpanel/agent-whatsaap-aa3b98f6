...
describe("generation lock recovery lease horizon", () => {
  it("keeps durable Stage B recovery at five minutes but gives synchronous generation owners a longer lease", () => {
    expect(recovery).toContain("const DEFAULT_STALE_MS = 5 * 60 * 1000");
    expect(recovery).toContain("const GENERATION_LOCK_STALE_MS = 20 * 60 * 1000");
