...
describe("Stage C zero-lost-turn static invariants", () => {
  ...
  it("keeps safe pre-runtime recovery sealed as retry_safe", () => {
    ...
    expect(customerTurnRecovery).toContain("WHEN safe_attempt_count>=5 THEN 'needs_review' ELSE 'retry_safe'");
    expect(customerTurnRecovery).toContain("state='processing_safe'");
  ...
  it("skips contended ready turns rather than blocking or returning on the first collision", () => {
    ...
    expect(effectiveClaim).toContain("CONTINUE;");
    expect(effectiveClaim).toContain("IF FOUND THEN RETURN; END IF;");
  });
  ...
  it("recovers stale standalone generation locks without crossing active runtime ownership", () => {
    ...
    expect(orphanLockRecovery).toContain("LIMIT 500");
    expect(orphanLockRecovery).toContain("EXIT WHEN v_locked>=100");
