...
describe("inbound Customer Turn claim barriers",()=>{
 it("historically blocks collecting while pending jobs remain unattached and review ingress is incomplete",()=>{expect(historical.match(/pending_job\.status='pending'/g)?.length??0).toBeGreaterThanOrEqual(4);expect(historical.match(/review_job\.status='needs_review'/g)?.length??0).toBeGreaterThanOrEqual(4);expect(historical).toContain("quarantine_customer_turns_with_unattached_review");});
