...
describe("public Agent V3 runtime entrypoints",()=>{
 ...
 it("historical public V3 test hook remains permanently inert",()=>{expect(testWebhook).toContain('new Response("not found", { status: 404 })');expect(testWebhook).not.toContain("V3_TEST_WEBHOOK_ENABLED");expect(testWebhook).not.toContain("runAgentV3Turn");expect(testWebhook).not.toContain("executeAgentV3Runtime");expect(testWebhook).not.toContain("supabaseAdmin");});
});
