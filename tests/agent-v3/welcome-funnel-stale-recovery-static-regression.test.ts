...
const migration=readFileSync("supabase/migrations/20260914434500_welcome_funnel_stale_recovery_canonical_lease.sql","utf8");
...
describe("stale Welcome Funnel recovery",()=>{
 it("quarantines uncertain runtime without replay and skips fresh leases under the canonical fence",()=>{expect(migration).toContain("status='needs_review'");expect(migration).toContain("pg_try_advisory_xact_lock(hashtextextended(v.conversation_id::text,31))");expect(migration).toContain("g.acquired_at>=p_generation_lock_stale_before");expect(migration).not.toContain("status='running' SET");});
