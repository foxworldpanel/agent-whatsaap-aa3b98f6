...
describe("Welcome Funnel partial failure semantics",()=>{
 ...
 it("quarantines unexpected post-start failures inside the runner",()=>{expect(source).toContain('catch(error){const message=error instanceof Error?error.message:String(error);try{await markExecutionNeedsReview');expect(source).toContain("Execução já terminal ou quarantine durável falhou após erro");});
 it("requires durable readback when the outer failure hook tries to quarantine running state",()=>{const hook=source.slice(source.indexOf("export async function markFunnelRunFailed"));expect(hook).toContain('.eq("status","running").select("funnel_id").maybeSingle()');expect(hook).toContain("else if(!data)");});
