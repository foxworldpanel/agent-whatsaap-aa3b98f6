...
> [!IMPORTANT]
> This phase focuses strictly on **public data extraction**. We will not implement login-based scraping, private data access, or AI enrichment in this sprint.

- Should we use a specific external API for Instagram public data, or start with a browser-based extraction strategy for the sandbox environment? (Default: Browser-based public profile extraction).
- Should the "Origin" selection for Instagram (Profile, Hashtag, Keyword) be visible but disabled for now, or hidden entirely? (Default: Visible but disabled except for "Profile").
