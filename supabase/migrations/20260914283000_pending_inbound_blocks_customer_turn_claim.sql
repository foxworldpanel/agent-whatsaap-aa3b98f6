-- A dispatcher attachment pass intentionally selects one representative per
-- conversation for fairness. Therefore a conversation can still have newer pending,
-- unattached Stage B jobs after its first job has joined a collecting Customer Turn.
-- Never seal/claim that semantic turn until all pending inbound work for the same
-- conversation has been attached. This prevents a burst from being split merely by
-- dispatcher batch boundaries.
...
BEGIN
 FOR v_candidate IN
  ...
  FROM (
   ...
   WHERE t.state='collecting' AND t.last_received_at<=p_quiet_before AND t.safe_attempt_count<5
     ...
     AND NOT EXISTS(SELECT 1 FROM public.agent_generation_locks generation_lock WHERE generation_lock.conversation_id=t.conversation_id)
     AND NOT EXISTS(
       SELECT 1 FROM public.agent_inbound_jobs pending_job
       WHERE pending_job.conversation_id=t.conversation_id AND pending_job.status='pending'
         AND NOT EXISTS(SELECT 1 FROM public.agent_customer_turn_messages tm WHERE tm.job_id=pending_job.id)
     )
 ...
 LOOP
  ...
  WHERE t.id=v_candidate.id AND t.conversation_id=v_candidate.conversation_id AND t.safe_attempt_count<5
    AND ((t.state='retry_safe' AND NOT EXISTS(SELECT 1 FROM public.agent_customer_turns older WHERE older.conversation_id=v_candidate.conversation_id AND older.state='retry_safe' AND (older.created_at,older.id)<(t.created_at,t.id)))
...
BEGIN
 ...
 WHERE t.id=p_turn_id AND t.conversation_id=v_conversation_id AND t.safe_attempt_count<5
   AND ((t.state='retry_safe' AND NOT EXISTS(SELECT 1 FROM public.agent_customer_turns older WHERE older.conversation_id=v_conversation_id AND older.state='retry_safe' AND (older.created_at,older.id)<(t.created_at,t.id)))
     OR (t.state='collecting' AND t.last_received_at<=p_quiet_before
       ...
       AND NOT EXISTS(
         SELECT 1 FROM public.agent_inbound_jobs pending_job
         WHERE pending_job.conversation_id=v_conversation_id AND pending_job.status='pending'
           AND NOT EXISTS(SELECT 1 FROM public.agent_customer_turn_messages tm WHERE tm.job_id=pending_job.id)
       )))
 RETURNING t.*;
END $$;
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$
 SELECT EXISTS(
  ...
  WHERE t.safe_attempt_count<5
    AND ((t.state='retry_safe' AND NOT EXISTS(SELECT 1 FROM public.agent_customer_turns older WHERE older.conversation_id=t.conversation_id AND older.state='retry_safe' AND (older.created_at,older.id)<(t.created_at,t.id)))
      OR (t.state='collecting' AND t.last_received_at<=p_quiet_before
