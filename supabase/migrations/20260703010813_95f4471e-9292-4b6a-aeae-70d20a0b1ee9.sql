...
UPDATE public.conversations old
SET status = 'encerrada', updated_at = now()
FROM public.contacts ct
JOIN public.conversations existing
  ON existing.contact_id = ct.id
 AND existing.whatsapp_number_id = ct.whatsapp_number_id
 AND existing.status IN ('agente_respondendo', 'aguardando')
WHERE old.contact_id = ct.id
  AND old.whatsapp_number_id IS NULL
  AND old.status IN ('agente_respondendo', 'aguardando')
  AND ct.whatsapp_number_id IS NOT NULL
  AND existing.id <> old.id;

UPDATE public.conversations c
SET whatsapp_number_id = ct.whatsapp_number_id, updated_at = now()
FROM public.contacts ct
...
WHERE whatsapp_number_id IS NULL
  AND status IN ('agente_respondendo', 'aguardando');
...
REVOKE ALL ON FUNCTION public.get_or_create_active_conversation(uuid, uuid, uuid, conversation_status) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_or_create_active_conversation(uuid, uuid, uuid, conversation_status) FROM anon;
REVOKE ALL ON FUNCTION public.get_or_create_active_conversation(uuid, uuid, uuid, conversation_status) FROM authenticated;
