-- An unattached Stage B job in needs_review is durable evidence that semantic input
-- for the conversation cannot be safely auto-executed. Do not let a collecting
-- Customer Turn silently pass it. Quarantine collecting turns behind such rows and
-- make claim/readiness treat the review barrier as non-runnable.
...
-- Refresh the effective Customer Turn claim/readiness functions with the review barrier.
 ...
 FOR v_candidate IN
  SELECT q.id,q.conversation_id,q.ready_at FROM (
   ...
   WHERE t.state='collecting' AND t.last_received_at<=p_quiet_before AND t.safe_attempt_count<5
     ...
     AND NOT EXISTS(SELECT 1 FROM public.agent_inbound_jobs review_job WHERE review_job.conversation_id=t.conversation_id AND review_job.status='needs_review' AND NOT EXISTS(SELECT 1 FROM public.agent_customer_turn_messages tm WHERE tm.job_id=review_job.id))
  ...
  WHERE t.id=v_candidate.id AND t.conversation_id=v_candidate.conversation_id AND t.safe_attempt_count<5
    AND ((t.state='retry_safe' AND t.updated_at<=now()-interval '15 seconds' AND NOT EXISTS(SELECT 1 FROM public.agent_customer_turns older WHERE older.conversation_id=v_candidate.conversation_id AND older.state='retry_safe' AND (older.created_at,older.id)<(t.created_at,t.id)))
      OR (t.state='collecting' AND t.last_received_at<=p_quiet_before
        ...
        AND NOT EXISTS(SELECT 1 FROM public.agent_inbound_jobs review_job WHERE review_job.conversation_id=v_candidate.conversation_id AND review_job.status='needs_review' AND NOT EXISTS(SELECT 1 FROM public.agent_customer_turn_messages tm WHERE tm.job_id=review_job.id))))
  RETURNING t.*;
 ...
 WHERE t.id=p_turn_id AND t.conversation_id=v_conversation_id AND t.safe_attempt_count<5
   AND ((t.state='retry_safe' AND t.updated_at<=now()-interval '15 seconds' AND NOT EXISTS(SELECT 1 FROM public.agent_customer_turns older WHERE older.conversation_id=v_conversation_id AND older.state='retry_safe' AND (older.created_at,older.id)<(t.created_at,t.id)))
     OR (t.state='collecting' AND t.last_received_at<=p_quiet_before
       ...
       AND NOT EXISTS(SELECT 1 FROM public.agent_inbound_jobs review_job WHERE review_job.conversation_id=v_conversation_id AND review_job.status='needs_review' AND NOT EXISTS(SELECT 1 FROM public.agent_customer_turn_messages tm WHERE tm.job_id=review_job.id))))
 RETURNING t.*;
END $$;
