-- Direct webhook attachment may wait for its own conversation fence, but the
-- dispatcher catch-up sweep must not let one busy conversation delay unrelated
-- pending jobs. Inline the same attachment invariants under a try-lock and scan
