...
-- changes into owner-checked RPCs sharing seed 31 with Customer Turns. Direct
-- Stage B finalizers must also refuse jobs already owned by a semantic turn.
...
BEGIN
 ...
 WHERE message_id=p_message_id AND conversation_id=v_conversation_id AND status='processing_safe' AND claimed_by=p_holder
 AND NOT EXISTS(SELECT 1 FROM public.agent_customer_turn_messages tm WHERE tm.job_id=agent_inbound_jobs.id)
...
BEGIN
 ...
 WHERE message_id=p_message_id AND conversation_id=v_conversation_id AND status='processing' AND claimed_by=p_holder
 AND NOT EXISTS(SELECT 1 FROM public.agent_customer_turn_messages tm WHERE tm.job_id=agent_inbound_jobs.id)
 RETURNING id INTO v_changed;
 RETURN v_changed IS NOT NULL;
END $$;
