-- A post-baseline legacy claim without durable execution state is not proof of
-- delivery. Materialize that ambiguity as an immutable durable needs_review
-- record while the shared generation lock and seed-31 fence are held, so pending
-- Stage B work cannot later bypass the webhook-only classification.
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1
    FROM public.welcome_funnel_legacy_claim_baseline
    WHERE funnel_id = p_funnel_id
      AND contact_id = p_contact_id
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM public.welcome_funnel_runs
    WHERE funnel_id = p_funnel_id
      AND contact_id = p_contact_id
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'ambiguous Welcome Funnel quarantine requires the generation lock'
      USING ERRCODE = '55000';
  END IF;
...
GRANT EXECUTE ON FUNCTION public.quarantine_ambiguous_welcome_funnel_claim(
  uuid, uuid, uuid, uuid, uuid
) TO service_role;
