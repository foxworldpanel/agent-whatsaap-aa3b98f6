...
-- Fail closed for nonterminal pre-migration turns, under the canonical per-
-- conversation fence so ingress/claim/recovery cannot race this quarantine.
DO $$
...
BEGIN
 ...
 LOOP
  ...
  WHERE t.id=v_candidate.id AND t.conversation_id=v_candidate.conversation_id
    AND t.state IN ('collecting','retry_safe','processing_safe','processing');
  IF FOUND THEN
