-- DISTINCT ON ordered by conversation_id makes the same first 100 stale
-- conversations dominate every recovery pass when they stay contended. Scan a
-- wider age-ordered conversation window and cap successfully acquired fences.
...
BEGIN
 ...
 FOR v_candidate IN
  SELECT grouped.conversation_id,grouped.oldest_claimed_at
  FROM (
    SELECT j.conversation_id,min(j.claimed_at) AS oldest_claimed_at
  ...
  ORDER BY grouped.oldest_claimed_at,grouped.conversation_id
  LIMIT 500
 LOOP
