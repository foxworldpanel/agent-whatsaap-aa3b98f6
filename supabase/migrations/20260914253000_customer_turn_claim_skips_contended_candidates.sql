-- A nonblocking advisory lock is only useful for queue fairness if claim_next can
-- continue to another candidate after a collision. Scan a bounded ready set and
-- try each conversation; one hot conversation must not make the worker report
-- idle while unrelated Customer Turns are immediately claimable.
...
BEGIN
 ...
 LOOP
  ...
  -- Discovery is advisory only. Revalidate every durable owner after the fence.
  IF EXISTS(
