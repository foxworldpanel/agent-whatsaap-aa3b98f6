-- A bounded job scan can still be monopolized by many rows from one contended
-- conversation. Pick the oldest eligible pending job per conversation first, then
-- globally age-order those representatives before attempting the shared seed-31
-- fence. Preserve every Customer Turn, active Stage B and generation-lock barrier.
