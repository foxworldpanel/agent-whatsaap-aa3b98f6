-- Preserve the burst-drain optimization without letting the oldest hot conversation
-- consume the whole global attachment budget. Each conversation gets a bounded slice
-- after winning the shared nonblocking fence; the outer sweep remains globally age-ordered.
...
BEGIN
 ...
 LOOP
  ...
  LOOP
    ...
    IF FOUND THEN
      v_count:=v_count+1;
      v_conversation_count:=v_conversation_count+1;
    END IF;
  END LOOP;

  IF v_turn IS NOT NULL AND v_conversation_count>0 THEN
