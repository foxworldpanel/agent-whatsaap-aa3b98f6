-- A corrupt cross-workspace collecting turn must not make the background drain throw
-- forever. Quarantine the conflicting durable turn and its already attached pending
-- members, leaving the unattached Stage B job durable for a later clean turn.
...
BEGIN
 FOR v_conversation IN SELECT c.conversation_id,c.workspace_id,c.created_at,c.id FROM(SELECT DISTINCT ON(j.conversation_id)j.conversation_id,j.workspace_id,j.created_at,j.id FROM public.agent_inbound_jobs j WHERE j.status='pending' AND NOT EXISTS(SELECT 1 FROM public.agent_customer_turn_messages tm WHERE tm.job_id=j.id) AND NOT public.has_welcome_funnel_agent_barrier(j.conversation_id,j.workspace_id) ORDER BY j.conversation_id,j.created_at,j.id)c ORDER BY c.created_at,c.id LIMIT least(v_target*10,1000) LOOP
  ...
  FOR v_job IN SELECT j.* FROM public.agent_inbound_jobs j WHERE j.conversation_id=v_conversation.conversation_id AND j.status='pending' AND NOT EXISTS(SELECT 1 FROM public.agent_customer_turn_messages tm WHERE tm.job_id=j.id) ORDER BY j.created_at,j.id FOR UPDATE OF j SKIP LOCKED LOOP
   ...
   IF v_turn IS NOT NULL AND v_turn_workspace IS DISTINCT FROM v_job.workspace_id THEN
    UPDATE public.agent_customer_turns SET state='needs_review',claimed_by=NULL,claimed_at=NULL,last_error='collecting Customer Turn workspace mismatch during pending attachment',updated_at=now() WHERE id=v_turn AND conversation_id=v_job.conversation_id AND state='collecting';
    IF FOUND THEN UPDATE public.agent_inbound_jobs j SET status='needs_review',claimed_by=NULL,claimed_at=NULL,last_error='attached Customer Turn routing identity mismatch',updated_at=now() FROM public.agent_customer_turn_messages tm WHERE tm.turn_id=v_turn AND tm.job_id=j.id AND j.status='pending';END IF;
    v_turn:=NULL;v_turn_workspace:=NULL;
   END IF;
