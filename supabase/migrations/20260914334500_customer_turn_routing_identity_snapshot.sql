-- Freeze the non-secret routing identity used by a sealed turn. Provider credentials
-- remain live/rotatable, but a contact/number/user reassignment must not silently
-- redirect already-accepted semantic work.
...
SET user_id=c.user_id,
    ...
    contact_phone=regexp_replace(coalesce(ct.telefone,''),'\D','','g')
FROM public.agent_inbound_jobs j
JOIN public.conversations c ON c.id=j.conversation_id
LEFT JOIN public.contacts ct ON ct.id=c.contact_id
WHERE tm.job_id=j.id
  AND (tm.user_id IS NULL OR tm.contact_id IS NULL OR tm.whatsapp_number_id IS NULL OR tm.contact_phone IS NULL);

CREATE OR REPLACE FUNCTION public.snapshot_agent_customer_turn_member()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE v_job public.agent_inbound_jobs%ROWTYPE; v_message record; v_conversation record; v_phone text;
BEGIN
 ...
 SELECT regexp_replace(coalesce(telefone,''),'\D','','g') INTO v_phone FROM public.contacts WHERE id=v_conversation.contact_id;
 ...
 NEW.whatsapp_number_id:=v_conversation.whatsapp_number_id; NEW.contact_phone:=v_phone;
 RETURN NEW;
END $$;
...
BEGIN
 ...
 OR NEW.contact_phone IS DISTINCT FROM OLD.contact_phone OR NEW.created_at IS DISTINCT FROM OLD.created_at THEN RAISE EXCEPTION 'Customer Turn member semantic snapshot is immutable'; END IF;
 RETURN NEW;
END $$;

CREATE OR REPLACE FUNCTION public.load_agent_customer_turn_members(p_turn_id uuid)
RETURNS TABLE(turn_id uuid,job_id uuid,message_id uuid,ordinal bigint,external_id text,input_text text,input_kind text,input_mime text,audio_url text,send_target text,deferred_funnel boolean,user_id uuid,contact_id uuid,whatsapp_number_id uuid,contact_phone text,resolved_text text,created_at timestamptz)
LANGUAGE sql SECURITY DEFINER SET search_path=public AS $$
 SELECT tm.turn_id,tm.job_id,tm.message_id,tm.ordinal,tm.external_id,tm.input_text,tm.input_kind,tm.input_mime,tm.audio_url,tm.send_target,tm.deferred_funnel,
        tm.user_id,tm.contact_id,tm.whatsapp_number_id,tm.contact_phone,tm.resolved_text,tm.created_at
