...
function sameInboundSnapshot(job: AgentInboundJob, input: AgentInboundJobInput): boolean {
  return (
    job.message_id === input.messageId && job.conversation_id === input.conversationId &&
...
export async function ensureAgentInboundJob(supabaseAdmin: any, input: AgentInboundJobInput): Promise<void> {
  const row = { message_id: input.messageId, conversation_id: input.conversationId, workspace_id: input.workspaceId,
    send_target: input.sendTarget, input_text: input.inputText, input_kind: input.inputKind,
    input_mime: input.inputMime ?? null, deferred_funnel: input.deferredFunnel, status: "pending" };
...
export async function claimAgentInboundJob(
  supabaseAdmin: any, messageId: string, holder: string,
  maxAttempts = AGENT_INBOUND_MAX_SAFE_ATTEMPTS,
): Promise<boolean> {
  const { data, error } = await supabaseAdmin.rpc("claim_agent_inbound_job", {
    p_message_id: messageId, p_holder: holder, p_max_attempts: maxAttempts,
  });
  if (error) throw error;
  return data === true;
}
...
export async function claimNextAgentInboundJob(
  supabaseAdmin: any, holder: string,
  maxAttempts = AGENT_INBOUND_MAX_SAFE_ATTEMPTS,
...
export async function transferAgentInboundJobClaim(supabaseAdmin: any, messageId: string, fromHolder: string, toHolder: string): Promise<boolean> {
  const { data, error } = await supabaseAdmin.rpc("transfer_agent_inbound_job_claim", {
    p_message_id: messageId, p_from_holder: fromHolder, p_to_holder: toHolder,
  });
...
export async function recoverStaleAgentInboundJobs(
  supabaseAdmin: any, staleBefore: string, maxAttempts = AGENT_INBOUND_MAX_SAFE_ATTEMPTS,
): Promise<{ requeued: number; review: number }> {
  const { data, error } = await supabaseAdmin.rpc("recover_stale_agent_inbound_jobs", {
    p_stale_before: staleBefore, p_max_attempts: maxAttempts,
  });
  if (error) throw error;
