...
export type AgentInboundJob = {
  id: string; message_id: string; conversation_id: string; workspace_id: string;
  send_target: string; input_text: string; input_kind: AgentInboundKind; input_mime: string | null;
  deferred_funnel: boolean; status: "pending" | "processing_safe" | "processing" | "processed" | "needs_review";
  claimed_by: string | null; claimed_at: string | null; attempt_count: number; last_error: string | null;
  created_at: string; updated_at: string;
};

type AgentInboundJobInput = { messageId: string; conversationId: string; workspaceId: string; sendTarget: string;
  inputText: string; inputKind: AgentInboundKind; inputMime?: string; deferredFunnel: boolean };

function sameInboundSnapshot(job: AgentInboundJob, input: AgentInboundJobInput): boolean {
  return job.message_id === input.messageId && job.conversation_id === input.conversationId &&
    job.workspace_id === input.workspaceId && job.send_target === input.sendTarget &&
    job.input_text === input.inputText && job.input_kind === input.inputKind &&
    (job.input_mime ?? null) === (input.inputMime ?? null) && job.deferred_funnel === input.deferredFunnel;
}

async function readAgentInboundJob(supabaseAdmin: any, messageId: string): Promise<AgentInboundJob | null> {
  const { data, error } = await supabaseAdmin.from("agent_inbound_jobs").select("*").eq("message_id", messageId).maybeSingle();
...
export async function ensureAgentInboundJob(supabaseAdmin: any, input: AgentInboundJobInput): Promise<void> {
  const { error } = await supabaseAdmin.from("agent_inbound_jobs").insert({ message_id: input.messageId,
    ...
    input_text: input.inputText, input_kind: input.inputKind, input_mime: input.inputMime ?? null,
    deferred_funnel: input.deferredFunnel, status: "pending" });
  if (!error) return;
  if (error.code !== "23505") throw error;
  const existing = await readAgentInboundJob(supabaseAdmin, input.messageId);
  if (!existing) throw new Error(`Agent inbound job duplicate disappeared for message ${input.messageId}`);
  if (!sameInboundSnapshot(existing, input)) throw new Error(`Agent inbound job snapshot conflict for message ${input.messageId}`);
}

export async function claimAgentInboundJob(supabaseAdmin: any, messageId: string, holder: string,
  maxAttempts = AGENT_INBOUND_MAX_SAFE_ATTEMPTS): Promise<boolean> {
  const { data, error } = await supabaseAdmin.rpc("claim_agent_inbound_job", { p_message_id: messageId,
    p_holder: holder, p_max_attempts: maxAttempts });
  if (error) throw error;
  return data === true;
}

export async function claimNextAgentInboundJob(supabaseAdmin: any, holder: string,
  maxAttempts = AGENT_INBOUND_MAX_SAFE_ATTEMPTS): Promise<AgentInboundJob | null> {
  const { data, error } = await supabaseAdmin.rpc("claim_next_agent_inbound_job", { p_holder: holder,
...
export async function recoverStaleAgentInboundJobs(supabaseAdmin: any, staleBefore: string,
  ...
  const { data, error } = await supabaseAdmin.rpc("recover_stale_agent_inbound_jobs", { p_stale_before: staleBefore,
    p_max_attempts: maxAttempts });
  if (error) throw error;
  const row = Array.isArray(data) ? data[0] : data;
  return { requeued: Number(row?.requeued ?? 0), review: Number(row?.review ?? 0) };
}