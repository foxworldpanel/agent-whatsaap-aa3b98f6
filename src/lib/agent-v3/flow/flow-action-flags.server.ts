...
/**
 * Checagem síncrona e barata: existe QUALQUER FlowAction ligada? Usada
 * antes de fazer o trabalho de computar OrderContext/FlowEngine mais
 * cedo no fluxo (pra influenciar a resposta) — enquanto tudo estiver
 * desligado (estado atual), essa função devolve false instantaneamente
 * e nenhum custo extra é gasto.
 */
export function anyFlowActionEnabled(): boolean {
  return Object.values(FLOW_ACTIONS_ENABLED).some((v) => v === true);
}
