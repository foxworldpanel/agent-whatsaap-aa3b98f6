...
export type AgentCustomerTurnMember = { turn_id:string; job_id:string; message_id:string; ordinal:number; external_id:string; input_text:string; input_kind:AgentInboundKind; input_mime:string|null; audio_url:string|null; send_target:string; deferred_funnel:boolean; resolved_text:string|null; created_at:string };
...
export async function quarantineIncompleteCustomerTurnSnapshots(s:any,limit=100):Promise<number>{const {data,error}=await s.rpc("quarantine_incomplete_agent_customer_turn_snapshots",{p_limit:limit});if(error)throw error;return Number(data||0);}
