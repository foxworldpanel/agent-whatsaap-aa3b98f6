      ...
      .upsert(
        {
          ...
          selector_intents: (data.selectorIntents ?? (current as any)?.selector_intents ?? []) as string[],
          selector_stages: (data.selectorStages ?? (current as any)?.selector_stages ?? []) as string[],
          selector_platforms: (data.selectorPlatforms ?? (current as any)?.selector_platforms ?? []) as string[],
          selector_products: (data.selectorProducts ?? (current as any)?.selector_products ?? []) as string[],
          selector_triggers: (data.selectorTriggers ?? (current as any)?.selector_triggers ?? []) as string[],
          selector_dependencies: (data.selectorDependencies ?? (current as any)?.selector_dependencies ?? []) as string[],
          selector_conflicts: (data.selectorConflicts ?? (current as any)?.selector_conflicts ?? []) as string[],
          version: newVersion,
