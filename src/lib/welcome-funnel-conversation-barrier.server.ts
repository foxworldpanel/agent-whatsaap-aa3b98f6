...
export async function getWelcomeFunnelConversationBarrier(supabaseAdmin:any,conversationId:string,userId:string,workspaceId:string):Promise<WelcomeFunnelConversationBarrier>{
 const{data,error}=await supabaseAdmin.rpc("get_welcome_funnel_conversation_barrier",{p_conversation_id:conversationId,p_user_id:userId,p_workspace_id:workspaceId});
