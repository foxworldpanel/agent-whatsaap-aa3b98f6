...
async function quarantineLegacyAmbiguousClaim(params: {
  supabaseAdmin: any; funnelId: string; contactId: string; conversationId: string;
  userId: string; workspaceId: string; holder: string;
}): Promise<void> {
  const { data, error } = await params.supabaseAdmin.rpc("quarantine_ambiguous_welcome_funnel_claim", {
  ...
  if (data !== true) throw new Error(`Ambiguous Welcome Funnel claim quarantine was not confirmed for ${params.funnelId}/${params.contactId}`);
}

export async function orchestrateWelcomeFunnel(params: {
  supabaseAdmin: any; funnel: WelcomeFunnelRuntime; contactId: string;
  conversationId: string; userId: string; workspaceId: string; phone: string;
  ...
  if (initial !== "legacy_ambiguous" && blocksAutomaticAgentAfterFunnelClassification(initial)) return { status: "blocked", classification: initial };
  if (initial !== "legacy_ambiguous" && !mayStartWelcomeFunnelExecution(initial)) throw new Error(`Unhandled Welcome Funnel classification: ${initial}`);
  ...
  try {
    ...
    if (fenced === "legacy_ambiguous") {
      await quarantineLegacyAmbiguousClaim({
        ...
        conversationId: params.conversationId, userId: params.userId, workspaceId: params.workspaceId, holder,
      });
      const quarantined = await classifyWelcomeFunnelExecution(params.supabaseAdmin, params.funnel.id, params.contactId);
      if (quarantined !== "durable_needs_review") throw new Error(`Ambiguous Welcome Funnel claim returned without durable review: ${quarantined}`);
