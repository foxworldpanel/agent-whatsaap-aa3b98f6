...
export function buildPromptV2(input) {
    ...
    // 6. Network module
    const networkModules = ['spotify_overview', 'spotify_playlist', 'spotify_followers', 'instagram', 'youtube', 'tiktok', 'facebook', 'kwai'];
    ...
    return {
        ...
        messages: [
            ...limitHistory(input.history).map(h => ({
                role: h.sender === 'agente' ? 'assistant' : 'user',
                content: h.body
            })),
...
function addBlock(blocks, name, content) {
    blocks.push({ name, content });
}
function buildConversationStateSummaryV2(state) {
    const lines = [];
    if (state.mode)
        lines.push(`Modo: ${state.mode}`);
    if (state.network && state.network !== 'unknown')
        lines.push(`Rede: ${state.network}`);
    if (state.service && state.service !== 'unknown')
        lines.push(`Serviço: ${state.service}`);
    if (state.currentStep)
        lines.push(`Etapa Comercial: ${state.currentStep}`);
    if (state.customer.hasAccount)
        lines.push('Cliente: Já possui cadastro');
    if (state.customer.hasBalance)
        lines.push('Saldo: Possui saldo em conta');
    if (state.freeTest.status !== 'none')
        lines.push(`Teste Grátis: ${state.freeTest.status}`);
    if (state.link)
        lines.push(`Link Informado: ${state.link}`);
    if (state.quantity)
...
function limitHistory(history, max = 6) {
    return history.slice(-max);
}
function validateTokenLimits(tokens, intent, warnings) {
