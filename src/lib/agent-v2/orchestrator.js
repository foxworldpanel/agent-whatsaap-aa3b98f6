...
async function persistTurnAnalytics(data) {
...
export async function runAgentV2Turn(input) {
    ...
    try {
        ...
        metrics.guardViolations = (guardResult?.violations.length || 0) + (regenerationResult ? regenerationResult.guardResult.violations.length : 0);
        ...
        if (phoneHash) {
            const analyticsEvent = {
                ...
                brainVersion: 'v2',
                builderVersion: '2.0.0',
                executionMode: (input.executionMode === 'isolated' ? 'isolated_test' : input.executionMode),
                ...
                fallbackUsed: false,
                stateChangedFields: Object.keys(stateAfterRouting).filter(k => stateAfterRouting[k] !== stateBefore[k]),
        ...
        const fallbackState = input.previousState || {
            workspaceId: input.workspaceId,
            conversationId: input.conversationId,
        ...
        return {
            ...
            routeResult: {
                ...
                detectedMode: (input.mode === 'outbound' ? 'outbound' : 'receptive'),
...
function applyStateEvents(state, events) {
    let newState = { ...state };
    for (const event of events) {
        switch (event.type) {
            ...
            case 'customer_account_known':
                newState.customer.hasAccount = event.value;
                break;
            ...
            case 'free_test_accepted':
                newState.freeTest.status = 'accepted';
                break;
            case 'free_test_waiting_link':
                newState.freeTest.status = 'waiting_link';
                break;
            case 'free_test_started':
                newState.freeTest.status = 'started';
                break;
            case 'free_test_completed':
                newState.freeTest.status = 'completed';
                break;
...
function generateDeterministicResponse(message, state, route) {
...
async function callBrainModel(input, prompt, model, instruction) {
    ...
    const messages = prompt.messages.map((m) => ({
        ...
        content: m.content
    })).filter((m) => m.role !== 'system'); // callLLMV2 handles systemPrompt separately
