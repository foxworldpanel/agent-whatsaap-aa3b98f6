 ...
 */
export function updateConversationStateV2(previousState, event) {
 ...
 */
export function resolveShortAnswerFromState(message, state) {
    ...
    if (lastQ.includes('rede') || lastQ.includes('network') || lastQ.includes('ask_network')) {
        if (normalized.includes('spotify'))
            return { type: 'network_detected', value: 'spotify' };
        if (normalized.includes('instagram') || normalized.includes('estragam'))
            return { type: 'network_detected', value: 'instagram' };
        if (normalized.includes('youtube'))
            return { type: 'network_detected', value: 'youtube' };
        if (normalized.includes('tiktok'))
            return { type: 'network_detected', value: 'tiktok' };
    }
    // Detecção de Serviço
    if (lastQ.includes('serviço') || lastQ.includes('service') || lastQ.includes('ask_service')) {
        if (normalized.includes('playlist'))
            return { type: 'service_detected', value: 'playlist' };
        if (normalized.includes('seguidor') || normalized.includes('followers'))
            return { type: 'service_detected', value: 'followers' };
        if (normalized.includes('curtida') || normalized.includes('likes'))
            return { type: 'service_detected', value: 'likes' };
    }
    // Detecção de Quantidade
    if (lastQ.includes('quantidade') || lastQ.includes('quantity') || lastQ.includes('ask_quantity')) {
        const match = normalized.match(/\d+/);
        if (match)
            return { type: 'quantity_detected', value: parseInt(match[0]) };
    }
    ...
    if (lastQ.includes('cadastro') || lastQ.includes('conta') || lastQ.includes('account')) {
        if (isYes)
            return { type: 'customer_account_known', value: true };
        if (isNo)
            return { type: 'customer_account_known', value: false };
    }
    if (lastQ.includes('pagamento') || lastQ.includes('payment')) {
        if (normalized.includes('pix'))
            return { type: 'payment_method_selected', value: 'pix' };
        if (normalized.includes('cartão') || normalized.includes('cartao') || normalized.includes('credit'))
            return { type: 'payment_method_selected', value: 'credit_card' };
    }
    return null;
}
/**
 ...
 */
export function buildConversationStateSummaryV2(state) {
    const summary = [];
    if (state.mode)
        summary.push(`mode: ${state.mode}`);
    if (state.network !== 'unknown')
        summary.push(`network: ${state.network}`);
    if (state.service !== 'unknown')
        summary.push(`service: ${state.service}`);
    if (state.intent !== 'unknown')
        summary.push(`intent: ${state.intent}`);
    if (state.customer.hasAccount)
        summary.push(`has_account: true`);
    if (state.customer.hasBalance)
        summary.push(`has_balance: true`);
    if (state.currentStep)
        summary.push(`current_step: ${state.currentStep}`);
    if (state.lastQuestion)
        summary.push(`last_question: ${state.lastQuestion}`);
    if (state.quantity)
        summary.push(`quantity: ${state.quantity}`);
    return summary.join('\n');
}
/**
 * Repositório em memória para testes
 */
export class InMemoryConversationStateRepositoryV2 {
    states = new Map();
