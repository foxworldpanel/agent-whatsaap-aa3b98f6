 ...
 */
export function calculateEstimatedCost(model, tokens, callDate = new Date().toISOString()) {
    if (!model)
        return { cost: 0 };
    ...
    const cacheCreationCost = ((tokens.cacheCreation || 0) / 1000000) * pricing.cacheCreationPricePerMillion;
    const cacheReadCost = ((tokens.cacheRead || 0) / 1000000) * pricing.cacheReadPricePerMillion;
 ...
 */
export function determineCustomerStage(intent, currentStep) {
    if (currentStep === 'conversation_closed')
        return 'closed';
    switch (intent) {
        ...
        default:
            if (currentStep.includes('registration'))
                return 'registration';
            if (currentStep.includes('recharge'))
                return 'recharge';
            if (currentStep.includes('order'))
                return 'ordering';
            return 'new_lead';
    }
}
 ...
 */
export function calculateQualityScores(flags) {
    const categories = {
 ...
 */
export function hashPhoneNumber(phone, workspaceId) {
