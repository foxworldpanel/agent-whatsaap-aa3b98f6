...
export async function generateAgentReplyWithMeta(params: {
  ...
  const system = [
    ...
    reengagementGreeting
      ? `⛔ VETO DE PRIORIDADE MÁXIMA — MODO REENGAJAMENTO APÓS HIATO ATIVO ⛔\nEste bloco SOBRESCREVE, nesta resposta, TODA a identidade abaixo, o EXEMPLO_MODELO_DISPARO, os REFINAMENTOS DE TOM CONSULTIVO, a ORDEM OBRIGATÓRIA do funil, qualquer regra de "interesse inicial pós-abertura", qualquer instrução de "vá direto para a pergunta de rede/serviço/quantidade" e QUALQUER lógica de disparo/blast/funil de vendas.\n\nCondição detectada: passaram VÁRIAS HORAS (ou virou o dia) desde a sua última mensagem, e o cliente voltou APENAS com uma saudação curta ("oi", "olá", "bom dia", "boa tarde", "boa noite", "tudo bem").\n\nOBRIGAÇÕES desta resposta (sem exceção, mesmo em thread de DISPARO):\n1) Retribua a saudação de forma curta e calorosa e se coloque à disposição. Exemplos: "Boa tarde! Como posso ajudar?" / "Oi! Tudo bem por aí? Como posso te ajudar hoje?" / "Bom dia! Vamos continuar de onde paramos, me conta o que você precisa 😊".\n2) PROIBIDO emendar/repetir/reformular na mesma mensagem QUALQUER pergunta pendente do funil (rede, serviço, quantidade, "qual desses você quer priorizar", CTA, link do painel, preço, teste grátis, ancoragem). NÃO importa que exista pergunta sua pendente logo acima no histórico.\n3) AGUARDE a próxima mensagem do cliente antes de retomar qualquer coisa. Só volte ao funil quando o cliente sinalizar concretamente o que quer.\n4) NÃO despache ===SPLIT===, NÃO envie link, NÃO cite preço, NÃO faça pergunta de negócio nesta resposta.`
      : "",
