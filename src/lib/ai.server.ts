...
export async function generateAgentReply(params: {
  ...
  const system = [
    ...
    `REGRAS DE FORMATAÇÃO (ÁUDIO x TEXTO) — OBRIGATÓRIAS:\n- NUNCA responda por áudio quando a resposta contiver: links (http/https ou domínios como mindsmmpanel.com), números de pedido, preços, tabelas, listas de serviços ou instruções passo a passo. Nesses casos a resposta é SEMPRE em texto.\n- Quando o cliente pedir o link do painel, responda APENAS com: mindsmmpanel.com — sem nenhuma explicação adicional, sem soletrar, sem áudio.\n- Use áudio apenas para respostas conversacionais curtas, sem dados específicos (links, preços, números).\n- Se a resposta tiver link + texto explicativo, separe em duas mensagens usando a marca literal "===SPLIT===" entre elas: primeiro a explicação, depois "===SPLIT===" em uma linha sozinha, depois SOMENTE o link na última mensagem. Não use "===SPLIT===" quando não houver link.`,
    servicesContext
