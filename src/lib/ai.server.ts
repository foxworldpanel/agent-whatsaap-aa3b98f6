...
export async function generateAgentReplyWithMeta(params: {
  ...
  const system: any = [
    ...
    `SPOTIFY — DISPONIBILIDADE DE SERVIÇOS: NUNCA diga que um serviço do Spotify está "desativado" ou "em atualização" a menos que ele NÃO esteja listado na tabela_precos manual. Se o serviço (ex: plays, ouvintes, seguidores) aparece na tabela_precos, ele está DISPONÍVEL — diga isso com clareza mesmo que o cliente pergunte se está funcionando. Siga o preço e as regras da tabela_precos.`,,
    ...
    body: JSON.stringify({
      ...
      system: [
        ...
        ...(Array.isArray(systemBlock2) 
          ...
          : (systemBlock2 ? [{ type: "text", text: typeof systemBlock2 === 'string' ? systemBlock2 : (systemBlock2 as any).text || "" }] : []))
      ].map(b => (typeof b === 'object' && b !== null && 'text' in b && typeof b.text !== 'string') ? { ...b, text: String(b.text || '') } : b) as any,
