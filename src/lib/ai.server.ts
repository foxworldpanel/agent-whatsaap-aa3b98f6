...
export async function generateAgentReplyWithMeta(params: {
  ...
  const system = [
    ...
    !isInbound ? `IMPORTANTE — GENERALIZAÇÃO DO GATILHO DE ABERTURA (ABSOLUTA): no EXEMPLO_MODELO_DISPARO acima o cliente responde "Sim" à pergunta de abertura. Isso é apenas UM exemplo de resposta afirmativa — a MESMA sequência obrigatória (pergunta de conexão pessoal → rede → serviço específico → preço com contexto → só então trata hesitação) deve disparar para QUALQUER resposta que demonstre interesse/aceitação em continuar a conversa, incluindo mas não limitada a: "sim", "pode sim", "pode falar", "pode mandar", "manda", "manda aí", "claro", "com certeza", "quero saber", "quero ver", "vai", "bora", "vamos", "beleza", "ok", "tá", "yes", "sure", "sí", ou QUALQUER variação afirmativa/receptiva similar — mesmo que a palavra exata seja diferente do exemplo. O que importa é a INTENÇÃO (aceitar continuar), não a palavra literal.\n\nNUNCA pule direto para oferecer um serviço específico (ex: "teste grátis de Reels", "1000 seguidores", "views no YouTube") só porque o cliente respondeu algo diferente de "Sim". Se o cliente ainda não mencionou rede nem serviço, você TAMBÉM não pode mencionar — comece SEMPRE pela pergunta de conexão pessoal da etapa 1 do EXEMPLO_MODELO_DISPARO.` : "",
